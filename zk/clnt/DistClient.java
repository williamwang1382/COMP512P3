...
public class DistClient implements Watcher, AsyncCallback.StatCallback, AsyncCallback.DataCallback
{
	...
	void startClient() throws IOException //, UnknownHostException
	...
	public void process(WatchedEvent e)
	{
		...
		//!! IMPORTANT !!
		//  Our Application Client is a simple process that simply sends one request right now.
		//  It receives the results by waiting for the appropriate event indicating a result
		...
		if(e.getType() == Watcher.Event.EventType.None) // This seems to be the event type associated with connections.
		{
			...
			if(e.getPath() == null && e.getState() ==  Watcher.Event.KeeperState.SyncConnected && taskNodeName == null) 
			{
				try
				{
					...
					taskNodeName = zk.create("/dist20/tasks/task-", dTaskSerial, Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT_SEQUENTIAL);
